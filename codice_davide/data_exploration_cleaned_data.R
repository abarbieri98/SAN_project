...
hist(rete1215_clean_deg, col="lightblue", xlim=c(0, 56), xlab="degree", ylab="Frequency")   
...
get.vertex.attribute(graph=rete1215_clean, index=top10_degree_1619[2]) #get the info about node

# ego network considering node present in both top 10
rete1619_clean_ego_degree <- induced.subgraph(rete1619_clean, neighborhood(rete1619_clean, order= 1, nodes= top10_degree_1619[2]) [[1]])
...
rete1215_clean_ego_degree <- induced.subgraph(rete1215_clean, neighborhood(rete1215_clean, order= 1, nodes= top10_degree_1215[1]) [[1]])
transitivity(rete1215_clean, type = "local", vids = top10_degree_1215[1])  #local clustering coefficient of node is 0.194, so 19.2% of neighbour nodes with distance 1 are linked
...
ei(rete1215_clean, vattr="ssd") #-0.43   #eterophily
ei(rete1619_clean, vattr="ssd") #-0.51   #eterophily

ei(rete1215_clean, vattr="h_fact") #0.35 #weak omophily
ei(rete1619_clean, vattr="h_fact") #0.36

ei(rete1215_clean, vattr="academic_fact") #0.48  #omophily
